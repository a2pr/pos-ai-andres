...
function preprocessImage(input){
    return tf.tidy(()=>{
        // converte imageBitmap para tensor
        const image = tf.browser.fromPixels(input)

        return tf.image
        .resizeBilinear(image, [INPUT_MODEL_DIMENTIONS, INPUT_MODEL_DIMENTIONS]) //redimensiona para array
        .div(255)   //normaliza os valores para 0,1
        .expandDims(0)  //adiciona dimensao batch
    })
}
...
function * processPrediction ({ boxes, scores, classes}, width, height){
    for(let index =0; index<scores.length; index++){
        ...
        if(scores[index] < CLASSES_THRESHOLD) continue
